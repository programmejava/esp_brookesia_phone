...
static player_ctx_t player_ctx = {0};
...
static void show_video_task(void *arg)
{
    ...
    if (player_ctx.canvas && player_ctx.out_buff) {
        ...
        ESP_LOGE(TAG, "Canvas or output buffer is NULL");
        bsp_display_unlock();
        goto err;
    }
...
err:
    ...
    if (player_ctx.auto_height && player_ctx.main) {
...
{
    ESP_RETURN_ON_FALSE(params, NULL, TAG, "Params cannot be NULL");
    ...
    // Initialize player context if not already initialized
    if (!player_ctx.is_init) {
        memset(&player_ctx, 0, sizeof(player_ctx_t));
    }
...
esp_err_t esp_lvgl_simple_player_wait_task_stop(int timeout_ms)
{
    ...
    uint32_t i = 0;
    while (player_task_handle != NULL && eTaskGetState(player_task_handle) != eDeleted && (timeout_ms < 0 || i < timeout_ms)) {
        vTaskDelay(pdMS_TO_TICKS(1));
        i++;
    }
    player_task_handle = NULL;

    if (timeout_ms >= 0 && i >= timeout_ms) {
        ESP_LOGE(TAG, "Player task stop timeout");
