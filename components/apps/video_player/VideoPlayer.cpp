...
#define APP_SUPPORT_AVI_FILE_EXT    ".avi"
...
void AppVideoPlayer::app_show_ui(void)
{
    ...
    /* Create player */
    if (_video_name == NULL) {
        ESP_LOGE(TAG, "No video file selected");
        return;
    }
    
    if (!isVideoFormatSupported(_video_name)) {
        ESP_LOGE(TAG, "Unsupported video format: %s", _video_name);
        return;
    }
    
    snprintf(_video_path, sizeof(_video_path), "%s/%s", BSP_SD_MOUNT_POINT, _video_name);
    ESP_LOGI(TAG, "Attempting to play video: %s", _video_path);
...
uint8_t AppVideoPlayer::searchMideaFiles(void)
{
    ...
    if (DIR *d = opendir(BSP_SD_MOUNT_POINT)) {
        while (struct dirent *dir = readdir(d)) {
            if (dir->d_type != DT_DIR && isVideoFormatSupported(dir->d_name)) {
    ...
    if (!_midea_info_vect.empty() && sel_file >= 0 && sel_file < _midea_info_vect.size()) {
        _video_name = _midea_info_vect[sel_file].video_name.c_str();
    } else if (!_midea_info_vect.empty()) {
        // Default to first video if sel_file is out of range
        _video_name = _midea_info_vect[0].video_name.c_str();
        sel_file = 0;
    } else {
...
bool AppVideoPlayer::isVideoFormatSupported(const char *filename)
{
    ...
    // AVI文件通常不被支持，但我们可以尝试
    if (name.find(".avi") != std::string::npos) {
        ESP_LOGW(TAG, "AVI format detected. This may not work as expected. Only Motion JPEG (.mjpeg) is fully supported.");
        return true; // 允许尝试，但给出警告
    }
    
    ESP_LOGE(TAG, "Unsupported video format: %s. Only .mjpeg files are supported.", filename);
    return false;
}
